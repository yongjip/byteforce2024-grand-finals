...
def randomize_usage(usage_pct, min_max_pct=0.1):
    return float(usage_pct * random.uniform(1.0 - min_max_pct, 1.0 + min_max_pct))



# create hotel amenity avg usage by hour data
def generate_hotel_avg_usage():
    hotel_avg_usage = []
    seed = 43
    random.seed(seed)
    day_of_week_list = hotel_reservation_df['day_of_week'].unique()

    for hotel_name, amenities in hotel_amenities.items():
        for day_of_week in day_of_week_list:
            for hour, usage_pct in hour_usage_pct.items():
                for amenity in amenities:
                    usage_pct = randomize_usage(usage_pct)
                    usage_cnt = int(avg_reservation_cnt_dict[hotel_name, day_of_week] * usage_pct)
                    hotel_avg_usage.append([hotel_name, day_of_week, hour, amenity, usage_cnt])

    hotel_avg_usage_df = pd.DataFrame(hotel_avg_usage, columns=['hotel_name', 'day_of_week', 'hour', 'amenity', 'usage_count'])
    hotel_avg_usage_df.sort_values(by=['hotel_name', 'amenity', 'day_of_week', 'hour'], inplace=True)
    return hotel_avg_usage_df
...
hotel_avg_usage_df = generate_hotel_avg_usage()
